      ...
      <el-table :data="papers" highlight-current-row  @selection-change="selsChange" v-loading="listLoading" style="width: 100%;">
        ...
        <el-table-column label="操作">
          <template scope="scope">
            <el-button size="small" type="success" @click="deletePaper(scope.row.id)">删除</el-button>
...
<script>
import {getPaperList, getPaperListByTitles, adminDeletePaper, getFile} from "../../api/api";
export default {
  data() {
    return {
      mode: '并模式',
  ...
  methods: {
    deletePaper(paper_id){
      adminDeletePaper({paper_id: paper_id}).then(res=>{
