...
<!--				</div>-->
        ...
        <!--导航菜单-->
				<el-menu :default-active="$route.path" class="el-menu-vertical-demo el-menu-expand" @open="handleopen" @close="handleclose" @select="handleselect"
					...
					<template v-for="(item,index) in $router.options.routes" v-if="!item.hiddenAdmin">
        ...
        <!--导航菜单-折叠后-->
				<ul class="el-menu el-menu-vertical-demo collapsed" v-if="collapsed" ref="menuCollapsed">
					<li v-for="(item,index) in $router.options.routes" v-if="!item.hiddenAdmin" class="el-submenu item">
...
<script>
import { adminSetpwd } from "../api/api";
export default {
  data() {
